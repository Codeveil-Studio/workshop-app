      ...
      <div className="space-y-2">
        ...
        <div className="relative inline-block w-full">
          <button
            ...
            className="w-full bg-white border-gray-200 border rounded-md pl-3 pr-8 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#29cc6a] focus:border-[#29cc6a] text-left text-gray-700 font-medium transition-all duration-200 hover:border-gray-300"
          >
            {activity.type}
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-[#29cc6a]">
              <svg 
                className={`h-5 w-5 transition-transform duration-200 ${isActivityTypeOpen ? 'rotate-180' : ''}`} 
                fill="none" 
                strokeLinecap="round" 
          ...
          {isActivityTypeOpen && (
            <div className="absolute z-10 w-full top-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg transform origin-top transition-all duration-150 max-h-48 overflow-y-auto">
              {['Inspection', 'Repair', 'Maintenance', 'Diagnostic'].map((type) => (
                <div
                  key={type}
                  onClick={() => {
                    handleActivityChange("type", type);
                  ...
                  className={`px-3 py-2 cursor-pointer hover:bg-[#29cc6a] hover:text-white transition-colors duration-150
                    ${activity.type === type ? 'bg-[#29cc6a] text-white' : 'text-gray-700'}
                    ${type === 'Diagnostic' ? 'rounded-b-md' : ''}
                    ${type === 'Inspection' ? 'rounded-t-md' : ''}`}
                >
                  {type}
                </div>
              ))}
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-[#29cc6a] text-white flex items-center justify-center hover:bg-[#22b85a] transition-colors duration-200 shadow-sm"
          >
        ...
        {showPaintCode && (
          <div className="space-y-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
            ...
            <button
              ...
              className="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-[#29cc6a] hover:text-[#29cc6a] transition-colors duration-200 flex items-center justify-center space-x-2"
            >
              <Plus size={16} />
